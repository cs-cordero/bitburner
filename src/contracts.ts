...
const CONTRACT_TYPE_TO_SCRIPT: {[contractType: string]: ((ns: NS, input: any) => string) | undefined } = {
...
export async function main(ns: NS): Promise<void> {
    const contractsToServer = getAllServers(ns)
        .flatMap(hostname => ns
            .ls(hostname)
        ...
        if (solver !== undefined) {
            ...
            if (ns.args.includes("--submit")) {
                ...
                if (result === "") {
                    ...
                    ns.tprint(`Successful contract execution on ${contract}@${server}: ${result}`)
                }
            } else {
                ns.tprint(`\t${contract}@${server} Answer: ${answer}`)
            }
        } else {
            ns.tprint(`${contract}@${server}: New contract type: "${contractType}"`)
