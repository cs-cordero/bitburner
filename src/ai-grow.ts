    ...
    if (formulasApiActive(ns)) {
        ...
        while (true) {
            ...
            for (const candidateServer of candidateServers) {
                ...
                if (threadsNeeded > 0) {
                    const createdProcs = executeGrowAndWeaken(ns, candidateServer, growThreadsNeeded, weakenThreadsNeeded)
        ...
        while (true) {
            ...
            targets.sort((a, b) =>
                (ns.getServerMaxMoney(b) * ns.hackAnalyzeChance(b)) - (ns.getServerMaxMoney(a) * ns.hackAnalyzeChance(a))
            )
...
function executeGrowAndWeaken(ns: NS, target: string, growThreads: number, weakenThreads: number): Process[] {
    ...
    print(`    Using ${growThreads + weakenThreads} threads across ${hostCount} hosts. Expected completion in ${expectedTime}`)
